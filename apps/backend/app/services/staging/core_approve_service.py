...
class CoreApproveService:
    ...
    def _assert_pending_approval(status_value: WorkflowStatus) -> None:
        if status_value != WorkflowStatus.pending_approval:
            raise HTTPException(
                ...
                detail="Only pending_approval records can be approved or rejected",
            )
    ...
    async def approve_record(self, db: AsyncSession, *, staging_id: UUID, note: str | None, current_user: User) -> MessageResponse:
        repo = CoreApproveRepository(db)
        ...
        if staging_obj.source_core_research_id is None:
            ...
            if core_obj is None or core_obj.deleted_at is not None:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Source core record not found")
