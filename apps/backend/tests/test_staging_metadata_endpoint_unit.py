...
def test_submit_for_review_commits_and_returns_message(client, sample_user, monkeypatch):
    _override_user_with_role(client, sample_user, "DATA_ENTRY")
    db = _FakeDbSession()
    client.app.dependency_overrides[get_db] = _fake_db_provider(db)
    staging_id = uuid4()
    ...
    assert response.status_code == 200
    assert response.json()["message"] == "Submitted for review"
...
def test_upload_endpoint_passes_file_and_access_level_to_service(client, sample_user, monkeypatch):
    ...
    async def _fake_upload(*args, **kwargs):
        file_bytes = await kwargs["file"].read()
        captured.update(kwargs)
    ...
    assert captured["staging_id"] == staging_id
    assert captured["access_level"] == "public"
    assert captured["file"].filename == "evidence.pdf"
    assert captured["file_bytes"] == b"pdf-content"
    assert db.commit_count == 1


def test_delete_staging_file_commits(client, sample_user, monkeypatch):
    _override_user_with_role(client, sample_user, "DATA_ENTRY")
    db = _FakeDbSession()
    client.app.dependency_overrides[get_db] = _fake_db_provider(db)
    staging_id = uuid4()
